<div class="external-stack">
    <section class="external-hero">
        <article class="external-hero__intro">
            ...
            <p>
                Cari gambar Unsplash lewat endpoint <code>/search/unsplash</code>. API key otomatis memakai
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_24rem]">
        <section class="surface-panel">
            <div class="surface-panel__header">
                <div>
                    <h3>Cari gambar Unsplash</h3>
                    <p class="surface-panel__text surface-panel__text--tight">
                        Masukkan keyword pencarian, lalu hasil akan ditampilkan sebagai kartu gambar, tabel, dan raw JSON.
                    </p>
                </div>
            </div>
                ...
                <div class="form-grid">
                    <div class="form-field form-field--wide">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <label for="unsplash_query" class="form-label">Query</label>
                                <p class="form-help">Contoh: <code>river in the mount</code>.</p>
                            </div>
                        ...
                        <input
                            ...
                            wire:model="query"
                            class="form-input"
                            placeholder="river in the mount"
                            autocomplete="off"
                        />
                ...
                <div class="result-stack">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        @forelse ($result['images'] as $image)
                            <article wire:key="unsplash-image-card-{{ md5($image['download'].$image['preview']) }}" class="overflow-hidden rounded-[1.6rem] border border-[rgb(var(--app-line))] bg-[rgb(246_245_240_/_0.55)]">
                                <div class="aspect-[4/3] overflow-hidden bg-white/70">
                                    @if ($image['download'])
                                        <img src="{{ $image['download'] }}" alt="{{ $image['title'] }}" class="h-full w-full object-cover" />
                                    @else
                                ...
                                <div class="space-y-3 p-5">
                                    <div class="space-y-2">
                                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-[rgb(var(--app-muted))]">Unsplash result</p>
                                    ...
                                    <div class="grid gap-3 rounded-[1.1rem] border border-[rgb(var(--app-line))] bg-white/70 p-4 text-sm text-[rgb(var(--app-muted))]">
                                        <div>
                                            <span class="block">Download URL</span>
                                            <strong class="mt-1 block break-all font-mono text-xs text-[rgb(var(--app-ink))]">{{ $image['download'] ?: '-' }}</strong>
                                        </div>
                                        <div>
                                            <span class="block">Preview URL</span>
                                            <strong class="mt-1 block break-all font-mono text-xs text-[rgb(var(--app-ink))]">{{ $image['preview'] ?: '-' }}</strong>
                                        </div>
                                    </div>
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <a href="{{ $image['download'] }}" target="_blank" rel="noopener noreferrer" class="primary-action w-full justify-center">
                                            Buka gambar
                                        </a>
                                        <a href="{{ $image['preview'] }}" target="_blank" rel="noopener noreferrer" class="primary-action w-full justify-center">
                                            Buka preview
                                        </a>
                                    </div>
                                </div>
                            ...
                            <table class="min-w-full divide-y divide-[rgb(var(--app-line))] text-left text-sm">
                                <thead class="bg-white/70 text-[rgb(var(--app-muted))]">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 font-semibold">Download</th>
                                        <th class="px-4 py-3 font-semibold">Preview</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-[rgb(var(--app-line))] bg-[rgb(246_245_240_/_0.45)]">
                                    @forelse ($result['images'] as $image)
                                        <tr wire:key="unsplash-image-row-{{ md5($image['download'].$image['preview']) }}">
                                            <td class="px-4 py-4 font-semibold text-[rgb(var(--app-ink))]">{{ $image['title'] ?: '-' }}</td>
                                            <td class="px-4 py-4">
                                                <a href="{{ $image['download'] }}" target="_blank" rel="noopener noreferrer" class="min-w-[20rem] break-all font-mono text-xs leading-6 text-[rgb(var(--app-accent))] hover:underline">
                                                    {{ $image['download'] ?: '-' }}
                                                </a>
                                            </td>
                                            <td class="px-4 py-4">
                                                <a href="{{ $image['preview'] }}" target="_blank" rel="noopener noreferrer" class="min-w-[20rem] break-all font-mono text-xs leading-6 text-[rgb(var(--app-accent))] hover:underline">
                                                    {{ $image['preview'] ?: '-' }}
                                                </a>
                                            </td>
                                        </tr>
        ...
        <aside class="settings-side">
            <section class="surface-panel">
                ...
                <div class="settings-checklist">
                    <article>
                        <strong>query</strong>
                        <p>Keyword pencarian gambar Unsplash, contoh <code>river in the mount</code>.</p>
                    </article>
                    <article>
            ...
            <section class="surface-panel">
                ...
                <div class="feature-list">
                    <article>
                        <h4>Card gallery</h4>
                        <p>Setiap hasil menampilkan gambar, judul, tautan download, dan tautan preview.</p>
                    </article>
                    <article>
                        <h4>Tabel link</h4>
                        <p>Semua URL juga dirender dalam tabel agar mudah discan atau disalin saat hasilnya banyak.</p>
                    </article>
                    <article>
                        <h4>Raw JSON</h4>
