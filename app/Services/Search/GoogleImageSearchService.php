...
class GoogleImageSearchService
{
    ...
    public const ENDPOINT = '/search/gimage';
    ...
    public function search(string $query): array
    {
        ...
        try {
            ...
            throw new RuntimeException(
                'Tidak dapat terhubung ke API search Google Image. Periksa koneksi internet atau coba beberapa saat lagi.',
                previous: $exception,
            );
    ...
    private function apiKey(): string
    {
        ...
        if (blank($apiKey)) {
            throw new RuntimeException(
                'API key Google Image search belum diatur atau tidak aktif. Tambahkan di Settings -> API Keys dengan name "'.self::API_KEY_NAME.'".',
            );
        }
    ...
    private function validateQuery(string $query): string
    {
        ...
        if (blank($query)) {
            throw new InvalidArgumentException('Query pencarian Google Image tidak boleh kosong.');
        }
    ...
    private function mapResponse(string $query, Response $response): array
    {
        ...
        if (! is_array($payload)) {
            throw new RuntimeException('API Google Image search mengembalikan response yang tidak valid (bukan JSON).');
        }

        if (Arr::get($payload, 'success') !== true || (int) Arr::get($payload, 'status', 0) >= 400) {
            throw new RuntimeException(
                (string) (Arr::get($payload, 'message') ?: 'API Google Image search mengembalikan status gagal.'),
            );
        }
    ...
    private function extractErrorMessage(?Response $response): string
    {
        ...
        if (is_array($payload)) {
            ...
            if (is_string($message) && filled($message)) {
                return 'API Google Image search error: '.$message;
            }
        }

        return 'API Google Image search error: request gagal dengan status '.($response?->status() ?? 'unknown').'.';
    }
}
